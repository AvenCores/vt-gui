...
* **Голосование (Лайк / Дизлайк):** Просмотр и отправка голосов за безопасность файла (Harmless / Malicious) с кэшированием статуса в реальном времени.
...
* **Тёмная и Светлая темы:** Полноценная поддержка тёмной (Dark Neon) и светлой (Light Clean) тем оформления для всех экранов, карточек, графиков и диалогов.
* **Быстрое переключение:** Кнопка смены темы прямо в шапке приложения рядом с настройками и селектор темы в окне настроек.
* **Сохранение настроек:** Выбранная тема сохраняется в `.env` и автоматически применяется при следующем запуске.
* **Плавная анимация:** Анимированная кнопка с вращением иконки солнца/луны на 180° с эффектом сжатия, и бесшовный плавный переход градиентов фона и элементов окна без мерцаний.
...
### 🌍 Локализация
* **12 языков:** Английский, русский, испанский, немецкий, французский, португальский, турецкий, украинский, китайский, японский, корейский и арабский языки с автоматическим определением языка системы.
...
* [source/main.py](source/main.py) — Главный файл приложения, точка входа, конфигурация окна Flet, обработка аргументов командной строки, управление темами и маршрутизация экранов.
* **`source/app/`** — Модульная архитектура приложения:
  * [source/app/strings.json](source/app/strings.json) — Файл локализации со всеми текстовыми строками интерфейса (12 языков, включая переводы для тем).
  * **`core/`** — Системное ядро и конфигурация:
    ...
    * [source/app/core/config.py](source/app/core/config.py) — Синхронизация окружения (`.env` и `~/.vt.toml`), сохранение тем (`THEME_MODE`), автоопределение языка, загрузка `strings.json`.
  ...
  * **`ui/`** — Графический интерфейс на Flet:
    * **`components/`** — Переиспользуемые визуальные компоненты:
      * [source/app/ui/components/theme.py](source/app/ui/components/theme.py) — Дизайн-система и цветовые токены палитр (`dark` и `light`), карточки статистики, строки антивирусов.
      * [source/app/ui/components/header.py](source/app/ui/components/header.py) — Верхняя панель (логотип, выбор языка, анимированная кнопка темы, настройки).
      * [source/app/ui/components/footer.py](source/app/ui/components/footer.py) — Нижняя панель (версия, ссылки, диалоги «О программе», «Донат», «Обновления»).
    * **`dialogs/`** — Модальные окна:
      * [source/app/ui/dialogs/api_key_dialog.py](source/app/ui/dialogs/api_key_dialog.py) — Диалог ввода API-ключа при первом запуске с поддержкой светлой/тёмной тем.
      * [source/app/ui/dialogs/settings_dialog.py](source/app/ui/dialogs/settings_dialog.py) — Окно настроек (API-ключ, выбор темы оформления, квоты, переустановка CLI).
    * **`views/`** — Экраны и вкладки приложения:
      ...
      * [source/app/ui/views/results_view.py](source/app/ui/views/results_view.py) — Панель результатов с вердиктами, поведением песочниц, комментариями и голосованием.
      ...
      * [source/app/ui/views/intelligence_view.py](source/app/ui/views/intelligence_view.py) — Аналитика угроз (URL, домены, IP-адреса, поиск по базе VT, DNS-резолвинг).
